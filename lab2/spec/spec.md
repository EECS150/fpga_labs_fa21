...
```
Build a 4-bit counter that will increment its value every second (and loop back to 0 once all 4 bits are used), and display the corresponding value on bits `3:0` of the IO LEDs. Some initial code has been provided in `counter.v` to help you get started.

## Simulation

### Simulating the Adder
Let's run some simulations on the counter in software to check it works before putting it on the FPGA.
...
Check the provided testbench skeleton in `sim/counter_testbench.v`.
...
### Analyzing the Simulation
However, you *may* see that your counter signal is just a red line. What's going on?
